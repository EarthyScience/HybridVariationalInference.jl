...
@testset "test_sample_zeta" begin
    ...
    @testset "sample_ζ_norm0 gpu" begin
        ...
        @test size(ζ_resid) == (length(ϕc.P) + n_θM * n_site,  n_MC)
        gr = Zygote.gradient(ϕc -> sum(CP.sample_ζ_norm0(rng, ϕc.P, ϕc.Ms, ϕc.unc; n_MC)[1]), ϕc)[1]; 
        @test length(gr) == length(ϕ) 
        int_unc = ComponentArrayInterpreter(ϕc.unc)
        gr2 = Zygote.gradient(ϕc -> sum(CP.sample_ζ_norm0(rng, CA.getdata(ϕc.P), CA.getdata(ϕc.Ms), CA.getdata(ϕc.unc), int_unc; n_MC)[1]), ϕc)[1]; 
    end;
