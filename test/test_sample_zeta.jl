...
function test_with_scenario(scenario)
    n_θP, n_θM  = length.(values(get_hybridproblem_par_templates(prob; scenario)))
    ...
    #cor_ends = (P=1:n_θP, M=[n_θM])   # need to tell that its zero parameters instead of 1
    ...
    @testset "transpose_Ms_sitefirst $(last(CP._val_value(scenario)))" begin
        x_true = collect(1:8)
    ...
    function test_sample_ζresid_norm(approx)    
        ...
        ζP_resids, ζMs_parfirst_resids, σ = @inferred CP.sample_ζresid_norm(
            approx, rng, i_sites_train, ϕc.Ms, ϕc.ϕq;
            n_MC=n_MC_pred, cor_ends, int_ϕq) 
        ...
        if ggdev isa MLDataDevices.AbstractGPUDevice
            @testset "sample_ζresid_norm gpu" begin
                ...
                i_sites_train = 1:n_site_batch
                ζP_resids, ζMs_parfirst_resids, σ = @inferred CP.sample_ζresid_norm(
                    approx, rng, i_sites_train, CA.getdata(ϕcd.Ms), CA.getdata(ϕcd.ϕq);
                    n_MC = n_MC_pred, cor_ends, int_ϕq)
               ...
               # ϕcd_few = CA.ComponentVector(ϕcd; Ms = ϕcd.Ms[:,1:n_site_few]);
                ...
                () -> begin
                    ...
                    CP.sample_ζresid_norm(
                    #@benchmark CP.sample_ζresid_norm(
                        approx, rng, i_sites_train, ϕcd.Ms, ϕcd.ϕq;
                        n_MC, cor_ends, int_ϕq)
                    #
                    ...
                    #@benchmark Zygote.gradient(ϕc -> begin # many small allocs
                        ζP_resids, ζMs_parfirst_resids, σ = CP.sample_ζresid_norm(
                            approx, rng, i_sites_train, CA.getdata(ϕc.Ms), CA.getdata(ϕc.ϕq);
                            n_MC, cor_ends, int_ϕq)
                        sum(ζP_resids) + sum(ζMs_parfirst_resids)
                    end, ϕcd_few)[1]
                end
    ...
    @testset "sample_ζresid_norm MeanHVIApproximationMat $(last(CP._val_value(scenario)))" begin
        approx = MeanHVIApproximationMat()
        test_sample_ζresid_norm(approx)        
    end

    @testset "sample_ζresid_norm MeanHVIApproximation $(last(CP._val_value(scenario)))" begin
        approx = MeanHVIApproximation()
        test_sample_ζresid_norm(approx)        
    end
