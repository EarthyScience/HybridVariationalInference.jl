...
@testset "generate_ζ" begin
    ζ, logdetΣ = CP.generate_ζ(
        rng, g, f, ϕcg_true, xM[:, 1:n_batch], map(get_concrete, interpreters);
        n_MC = 8)
    @test ζ isa Matrix
    gr = Zygote.gradient(
        ϕ -> sum(CP.generate_ζ(
...
@testset "neg_elbo_transnorm_gf cpu" begin
    cost = neg_elbo_transnorm_gf(rng, g, f, ϕcg_true, y_o[:, 1:n_batch], xM[:, 1:n_batch],
