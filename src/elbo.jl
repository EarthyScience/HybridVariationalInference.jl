...
- `xM`, `xP`, `y_ob`, `y_unc`, `i_sites_train`: information of the sites in the current minibatch
    ...
    - `i_sites_train`: indices of sites incurrent minibatch within training dataset
...
function neg_elbo_gtf_components(rng, ϕ::AbstractVector{FT}, g, f, py,
    xM::AbstractMatrix, xP, y_ob, y_unc, i_sites_train::AbstractVector{<:Number};
    ...
    ζsP, ζsMs_tr, σ = generate_ζ(approx, rng, g, ϕ, xM; n_MC, cor_ends, pbm_covar_indices,
        int_ϕq, int_ϕg_ϕq, is_testmode, i_sites_train, ranef)
    ...
    loss_comps = neg_elbo_ζtf(
        ...
        i_sites_train, intθMs, intθP, ranef, frac_cluster_all)
end
...
function neg_elbo_ζtf(ζsP::AbstractArray{T}, ζsMs_tr, σ, f, py, xP, y_ob, y_unc;
    ...
    frac_cluster = if iszero(i_sites_train[1]) 
        # unknonw site indices, e.g. test deata
         zeros(eltype(frac_cluster_all),length(i_sites_train)) 
    else
    ...
    f_sample = (ζP, ζMs_tr) -> begin
            ...
            f_sample_pre(logjac_P, logjac_Ms, θP, θMs_tr, y_pred_i, addq_pred_i; 
                py, y_ob, y_unc,
                penalty_computer, i_sites_train, ϕqc,
                intθMs, intθP,
            ...
            # # Test.@inferred compute_penalty(penalty_computer, y_pred_i, addq_pred_i, intθMs(θMs_tr), intθP(θP), i_sites_train, ϕq)[1]
            # # loss_penalty_i = convert.(typeof(nLy_i),first(compute_penalty(penalty_computer,
            # #     y_pred_i, addq_pred_i, intθMs(θMs_tr), intθP(θP), i_sites_train, ϕq)))
            # loss_penalty_i = compute_penalty(penalty_computer,
            #     y_pred_i, addq_pred_i, intθMs(θMs_tr), intθP(θP), i_sites_train, ϕqc)[1]
...
function average_elbo_components_across_samples(nLys, nLpriors_P, nLpriors_M, 
    ...
    # if i_sites_train[1] == 1
    #     #Main.@infiltrate_main
    #     @show nLy, entropy_ζ, nLmean_θ, n_MC, n_MC_cap, i_sites_train[1:3]
...
If predicting for a subset of training sites, specify keyword argument `i_sites_train`,
...
function predict_hvi(rng::AbstractRNG, prob::AbstractHybridProblem; scenario=Val(()), 
    ...
    xM, xP, i_sites_train, frac_cluster = initX_from_prob(prob; scenario, gdevs,  
        xM, xP, i_sites_train, frac_cluster)
    ...
    (; θsP, θsMs_tr, entropy_ζ, ζsP, ζsMs_tr, logjacs_P, logjacs_Ms) = sample_posterior(
        rng, prob, xM; 
        scenario, gdevs, is_testmode, i_sites_train, n_sample_pred, n_sample_ranef,
        frac_cluster,
        kwargs...)
    #
    ...
    penalties_sum = mapreduce((x,y) -> x .+ y, axes(y,3)) do i_MC
        CA.ComponentVector(
            ...
            y[:,:,i_MC], addq[:,:,i_MC], intθMs(θsMs_tr[:,:,i_MC]), intθP(θsP[:,i_MC]), i_sites_train, 
            prob.ϕq)
...
function initX_from_prob(prob; scenario, gdevs, xM, xP, i_sites_train, frac_cluster)
    if isnothing(xM) || isnothing(xP)
        ...
        i_sites_train = isempty(i_sites_train) ? (1:size(xM_dl, 2)) : i_sites_train
        xP = isnothing(xP) ? xP_dl[:,i_sites_train] : xP
        xM = isnothing(xM) ? xM_dl[:,i_sites_train] : xM
    end
...
rng::AbstractRNG, prob::AbstractHybridProblem; scenario=Val(()), 
    gdevs = get_gdev_MP(scenario), 
    is_testmode = true,
    i_sites_train = 1:get_hybridproblem_n_site_and_batch(prob; scenario)[1],
    n_sample_pred = 200,
    n_sample_ranef = n_sample_pred ÷ 10,
    frac_cluster_all = 
    ...
    ii_sites = map(x -> findfirst(==(x), i_sites_train), i_sites_train)
    ...
    res_predict = a = predict_hvi(rng, prob; 
            scenario, gdevs, xP, xM, is_testmode, i_sites_train, 
...
function sample_posterior(rng::AbstractRNG, prob::AbstractHybridProblem; 
    i_sites_train=Int[], scenario=Val(()), 
    gdevs = get_gdev_MP(scenario),
    kwargs...
    )
    dl = get_hybridproblem_train_dataloader(prob; scenario)
    ...
    if isnothing(i_sites_train) || isempty(i_sites_train)
        xM = xM_all
        i_sites_train = 1:size(xM, 2)
    else
        xM = xM_all[:,i_sites_train]
    end
    sample_posterior(rng, prob, xM; scenario, gdevs, i_sites_train,kwargs...)
end
...
function sample_posterior(rng::AbstractRNG, g, ϕ::AbstractVector, xM::AbstractMatrix;
    ...
    Test.@inferred(generate_ζ(approx, rng, g, CA.getdata(ϕ), CA.getdata(xM);
        ...
        cor_ends, pbm_covar_indices, is_testmode, i_sites_train, ranef,
        )) :
...
function generate_ζ(
    ...
    ζP_resids, ζMs_parfirst_resids, σ = sample_ζresid_norm(approx, rng, 
        i_sites_train, ϕm0, ϕq; n_MC, cor_ends, int_ϕq)
    # if i_sites_train is given add known random effect to μ_ζMs_g0
    ...
    # if random effect is kown, i.e. i_sites_train is provided, add to ζMs_resids
    #    otherwise add the same sampled random effect for n_sample_ranef sites to ζMs_resids
    ζMs_parfirst_ranef_resids = if !isempty(i_sites_train) && !iszero(i_sites_train[1])
        ...
        add_ranef(ranef, ζMs_parfirst_resids, ϕq_ranef, i_sites_train) 
    else
...
function sample_ζresid_norm(
    ...
    sample_ζresid_norm(approx, i_sites_train, zP, zMs, CA.getdata(ϕm), ϕq, args...;
        cor_ends, 
